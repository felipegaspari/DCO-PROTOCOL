...
static constexpr uint32_t SERIAL_DMA_TIMEOUT_MS = 2; // Leaky Pipe Timeout Fix

static inline bool serial2_dma_tx_ready() { return true; }
...
#endif
    size_t avail = (e.len[e.fill] < SERIAL_DMA_BUF_SIZE) ? (size_t)(SERIAL_DMA_BUF_SIZE - e.len[e.fill]) : 0;
#if defined(ARDUINO_ARCH_RP2040)
    mutex_exit(&e.mutex);
#endif
    return avail;
...
inline size_t UartDmaTx::write(const uint8_t *p, size_t n) {
  ...
  while (true) {
    ...
    if ((millis() - start_time) > SERIAL_DMA_TIMEOUT_MS) {
      return 0;
    }
  }
}
